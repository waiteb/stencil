    ...
    nodes.forEach(function(node) {
        ...
        switch (node.nodeType) {
            ...
            case Stencil.NodeType.ELEMENT: {
                output += this.prerenderElement(model, node);
                break;
            }
 ...
 * 
 */
Stencil.prototype.prerenderElement = function(model, node) {
    ...
    while (attributeList.length && !output) {
        ...
        if (attribute.handler.onRender) {
            output = attribute.handler.onRender(model, attribute, node, this);
        }
    }
    ...
    if (output === undefined) {
        output = this.renderElement(model, attributes, node);
    }
 ...
 * 
 */
Stencil.prototype.renderElement = function(model, attributes, node) {

    var children = this.render(model, node),
        output;
