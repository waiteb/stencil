/* jshint node: true, esversion: 6, undef: true, unused: true, browser: true */
...
/**
 * Settings used when parseing a DOM
 * @private
 */
 ...
 * @public
 */
Stencil.prototype.render = function(model, boundElement) {
    
    var scratchpad,
        html;
    
    scratchpad = new Scratchpad(this.globals, boundElement);
    scratchpad.push(this.document, model, this);
    
    html = this.renderChildren(scratchpad);

    // If we're binding an element, then we need to cache 
    // the virtual dom:
    if (boundElement) {
        this.virtualDom = scratchpad.virtualDom;        
    }
...
Stencil.prototype.renderElement = function(scratchpad) {
    ...
    for (let i = 0, length = node.attributePlugins.length; i < length && !output; i ++) {
        ...
        let attribute = node.attributePlugins[i],
            ...
            value = this.renderExpression(scratchpad.model, attribute.expression, scratchpad);
    ...
    if (output === undefined) {
        ...
        let attributes = this.renderExpression(scratchpad.model, node.attributesExpression, scratchpad);
...
Stencil.prototype.renderExpression = function(model, expression, scratchpad) {
    ...
    if (expression.type === Stencil.ExpressionType.BINARY || expression.type === Stencil.ExpressionType.LOGICAL) {
        ...
        if (strategy.renderLeft === undefined || strategy.renderLeft) {
            expression.left = expression.left && this.renderExpression(model, expression.left, scratchpad);
        }
        if (strategy.renderRight === undefined || strategy.renderRight) {
            expression.right = expression.right && this.renderExpression(model, expression.right, scratchpad);
        }
    }
...
/**
 * Sets up two-way binding for the stencil
 * 
 * @param  {Element} element
 * @param  {Object} model
 */
Stencil.prototype.bind = function(element, model) {
    
    var html = this.render(model, element);
...
/**
 * Updates the DOM tree that belongs to the given model.
 * 
 * @param  {Object} model The model that was updated
 * @param  {Object} node (optional) The node to recursively traverse
 */
Stencil.prototype.update = function(model, node) {

    node = node || this.virtualDom;
    ...
    for (let i = 0, length = node.children.length; i < length; i++) {
        ...
        if (node.model === model) {
            node.children[i] = this.updateNode(model, node);
        }
        else {
            this.update(model, node);
        }
    }
};
...
/**
 * Renders and updates the given node in the DOM and virtual DOM.
 * 
 * @param  {Object} model
 * @param  {Object} virtualNode
 */
Stencil.prototype.updateNode = function(model, virtualNode) {


    // Find the matching virtual node in the DOM:
    var element = this.findElementFor(virtualNode),
        scratchpad = new Scratchpad(virtualNode.scope, element),
        html,
        index,
        newElement;
    
    // Recreate the scratchpad for the virtual node, and render it:
    scratchpad.push(virtualNode.node, virtualNode.model, virtualNode.stencil);
    html = this.renderChildren(scratchpad);

    // Update both the DOM...
    element.innerHTML = html;
    
    // TODO:  I swear to god that this is needed, but it doesn't work, and commenting it
    // out allows things to work.  So, WTF.

    // ...and update the virtual DOM:
    // index = virtualNode.parent.children.indexOf(virtualNode);
    // virtualNode.parent.children[index] = scratchpad.virtualDom.children[0];
};

/**
 * Finds the matching element in the DOM given a virtual virtualNode:
 * 
 * @param  {Object} virtualNode A virtual node
 * @returns {DOMNode} The DOM node for the virtual node
 */
Stencil.prototype.findElementFor = function(virtualNode) {
    ...
    if (parent) {
        ...
        // ...find the element that matches this node...
        matchingNode = this.findElementFor(parent);

        // ...then figure out what index this node is, and return
        // the matching child element in the DOM:
        index = parent.children.indexOf(virtualNode);
        result = matchingNode.childNodes[index];
    }

    // If we don't have a parent, then we're at the top of the DOM:
    else {
        result = {childNodes: [this.boundElement]}; 
    }
