 ...
 */
Stencil.addExpression = function(expression) {
    ...
    key = Stencil.expressionNameFor(expression.name, operator);
    Stencil.expression[key] = expression;
};
Stencil.expression = {};
 ...
 */
Stencil.addElement = function(element) {
    Stencil.element[element.tagName] = element;
};
Stencil.element = {};
require("./stencil.elements.js")(Stencil);
...
Stencil.prototype.include = function(name, substencil) {
    ...
    // Transform the arguments into a dictionary if
    // if key/value mode is used:
    if (typeof name === "string") {
        substencils[name] = substencil;
    }
    else {
        substencils = name;
    }
    ...
    Object.keys(substencils).forEach(function(key) {
        ...
        var stencil = substencils[key];

        // If the stencil is HTML, create a new stencil using 
        // the same settings as this stencil:
        if (typeof stencil === "string") {
            stencil = new Stencil(stencil, this.settings);
        }

        // Add the substencil to the dictionary:
        this.substencils[key] = stencil;
...
Stencil.prototype.renderElement = function(model, node, scratchpad) {
    ...
    var attributeList = this.renderAttributes(model, node, scratchpad),
        attribute,
        attributes = [],
        element = Stencil.element[node.tagName],
        output;
...
Stencil.prototype.renderExpression = function(model, expression, scratchpad) {
    ...
    var key = Stencil.expressionNameFor(expression.type, expression.operator),
        strategy = Stencil.expression[key].callback || function() {},
        output;
