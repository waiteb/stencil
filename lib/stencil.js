 ...
 * @param  {Object} node
 */
Stencil.prototype.updateNode = function(model, node) {


    // Find the matching node in the DOM:
    var element = this.findElementFor(node),
        scratchpad = new Scratchpad(node.scope, element),
        html,
        index,
        newElement;
    
    // Recreate the scratchpad for the node, and render it:
    scratchpad.push(node.node, node.model, node.stencil);
    html = this.renderChildren(scratchpad);
    ...
    node.parent.children[index] = scratchpad.virtualDom.children[0];
};

/**
 * Finds the matching element in the DOM given a virtual node:
 * 
 * @param  {Object} node A virtual node
 * @returns {DOMNode} The DOM node for the virtual node
 */
Stencil.prototype.findElementFor = function(node) {
