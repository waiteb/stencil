...
Stencil.prototype.branch = function(stencil, scratchpad) {
    ...
    // Pop the current node from the stack, and replace the 
    // node (without pushing).  This will allow the branch 
    // stencil to take its place in the stack:
    node = scratchpad.pop();
    scratchpad.node = stencil.document;
    
    // Render the new node's children:
