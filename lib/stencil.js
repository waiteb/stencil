...
Stencil.prototype.compileExpressions = function(node) {        
    ...
    Object.keys(node.attributes || []).forEach(function(key) {
        ...
        // Assign the expression to the attribute if it exists:
        if (expression) {
 ...
 * @param {object} optionalNode The node to start rendering from.  If not provided, the entire document will be rendered.
...
Stencil.prototype.renderElement = function(model, node) {
    ...
    var attributeList = this.renderAttributes(model, node),
        attribute,
        attributes = [],
        output;
        
    // Clone the node so we don't need to worry about mutating it:
    // PERF: Keep an eye on this...
    ...
    for (let i = 0; i < attributeList.length && !output; i ++) {
        ...
        if (attribute.handler.onRender) {
            output = attribute.handler.onRender(attribute, model, node, this);
        }
    }
 ...
 */
Stencil.prototype.renderAttributes = function(model, node) {
