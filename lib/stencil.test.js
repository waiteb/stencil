...
describe("A stencil", function() {
    ...
    xit("throws an exception when not constructed with html", function() {
        // How?
    });
    ...
    describe("has custom attributes that", function() {
        ...
        it("can be sorted in the rendered html", function() {

            var html = '<div gamma="" beta="" alpha=""></div>',
                stencil,
                output;

            Stencil.addAttribute({name: "alpha", order: 0, show: true});
            Stencil.addAttribute({name: "beta", order: 1, show: true});
            Stencil.addAttribute({name: "gamma", order: 2, show: true});
