/* jshint node: true, esversion: 6, undef: true, unused: true, jasmine: true, browser: true */
...
describe("A stencil", function() {
    ...
    xit("can include substencils that are node modules", function() {
        ...
        rootStencil.include({alpha: "npm://stencil-module"});
    ...
    describe("two-way binding", function() {
        ...
        it("can bind a stencil to to DOM", function() {

            var stencil = new Stencil("{{value}}"),
                model = {value: "test"},
                element = document.createElement("div");

            stencil.bind(element, model);

            expect(element.innerHTML).to.equal("test");
            expect(stencil.virtualDom).to.be.an("object");
        });

        it("can update a bound model", function() {

            var stencil = new Stencil("<div>{{alpha}}<p>{{beta}}<span>{{gamma}}</span></p></div>"),
                model = {alpha: "alpha", beta: "beta", gamma: "gamma"},
                root = document.createElement("div");

            stencil.bind(root, model);
            
            model.gamma = "gamma!";
            stencil.update(model);

            expect(root.innerHTML).to.equal("<div>alpha<p>beta<span>gamma!</span></p></div>");
        });
