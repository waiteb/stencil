...
function Scratchpad(document, rootScope, stencil) {
     ...
     * @param  {Stencil} stencil The stensil responsible for rendering the node
     */
    function State(node, stencil, scope) {
        this.node = node;
        this.stencil = stencil;
        this.parent = stack[stack.length - 1];
        this.model = this.parent && this.parent.model || {};
        this.scope = scope || currentState.scope;
        this.element = {};
    }
    ...
    State.prototype.newScope = function() {
        ...
        // If we haven't already created a scope for this state, create one:
        if (!this.scopeLatch) {
    ...
    currentState = new State(document, stencil, Object.create(rootScope || {}));

    /**
     * Pushes a new node onto the stack.
     * 
     * @param {object} node The new node
     * @param {Stencil} node The stencil rendering the node
     * @private
     */
    function push(node, stencil) {
        stack.push(currentState);
        currentState = new State(node, stencil);
    }
