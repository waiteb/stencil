...
describe("scratchpad", function() {
    ...
    it("may be constructed with a root element (for two-way binding)", function() {

        var rootScope = {},
            boundElement = {},
            scratchpad = new Scratchpad(rootScope, boundElement);

        expect(scratchpad.boundElement).to.equal(boundElement);
    });
    ...
    it("creates a virtual DOM when binding to the DOM", function() {

        var scope = {},
            element = {},
            scratchpad = new Scratchpad(scope, element),
            tree;

        // Simulate a node containing another node:
        scratchpad.push("node1", "model1", "stencil1");
            scratchpad.push("node2", "model2", "stencil2");
            scratchpad.pop();
        scratchpad.pop();

        tree = scratchpad.virtualDom;

        expect(tree).to.be.an("object"); // root
        expect(tree.node).to.equal(undefined);
        expect(tree.model).to.equal(undefined);
        expect(tree.stencil).to.equal(undefined);

        tree = tree.children[0]; // root.node1
        expect(tree.node).to.equal("node1");
        expect(tree.model).to.equal("model1");
        expect(tree.stencil).to.equal("stencil1");
        
        tree = tree.children[0]; // root.node1.node2
        expect(tree.node).to.equal("node2");
        expect(tree.model).to.equal("model2");
        expect(tree.stencil).to.equal("stencil2");
    });
    

    it("can update a model in the virtual DOM", function() {
        ...
        // Simulate a node containing another node:
        scratchpad.push("node1", "model1", "stencil1");
        scratchpad.model = "model1.1";
        scratchpad.pop();

        tree = scratchpad.virtualDom;

        tree = tree.children[0];
        expect(tree.model).to.equal("model1.1");        
    });
});